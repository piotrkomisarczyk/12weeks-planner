...
export const GET: APIRoute = async ({ locals, request }) => {
  try {
    ...
    try {
      const weeklyGoals = await weeklyGoalService.listWeeklyGoals(validationResult.data, userId);
...
export const POST: APIRoute = async ({ locals, request }) => {
  try {
    ...
    try {
      const weeklyGoal = await weeklyGoalService.createWeeklyGoal(userId, validationResult.data);

      // Step 5: Return successful response
      return new Response(JSON.stringify({ data: weeklyGoal } as ItemResponse<WeeklyGoalDTO>), {
        status: 201,
        headers: {
          "Content-Type": "application/json",
          "X-Content-Type-Options": "nosniff",
