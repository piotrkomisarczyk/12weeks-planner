...
import type { ErrorResponse, ValidationErrorResponse, ListResponse, ItemResponse, MilestoneDTO } from "../../../types";
...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const result = await milestoneService.listMilestones(validatedParams, userId);
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const milestoneService = new MilestoneService(locals.supabase);
    const milestone = await milestoneService.createMilestone(validatedData, userId);
