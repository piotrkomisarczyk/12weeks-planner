...
export const GET: APIRoute = async ({ locals, request }) => {
  try {
    ...
    try {
      const weeklyReviews = await weeklyReviewService.listWeeklyReviews(
...
export const POST: APIRoute = async ({ locals, request }) => {
  try {
    ...
    try {
      const weeklyReview = await weeklyReviewService.createWeeklyReview(
        userId,
        validationResult.data
      );
      ...
      // Step 5: Return successful response
      return new Response(
        JSON.stringify({ data: weeklyReview } as ItemResponse<WeeklyReviewDTO>),
        {
          status: 201,
          headers: {
            'Content-Type': 'application/json',
