                  ...
                  {availableGoals.map((goal) => (
                    <button
                      ...
                      className={cn(
                        ...
                        currentGoalId === goal.id ? "border-primary bg-accent" : "border-border"
                      )}
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <Badge
                          className={cn("shrink-0", GOAL_CATEGORY_COLORS[goal.category] || "bg-gray-500 text-white")}
                        >
                          {getCategoryLabel(goal.category)}
                        </Badge>
                        {currentGoalId === goal.id && <CheckCircle2 className="h-5 w-5 text-primary shrink-0" />}
                      </div>
                    </button>
                  ))}
                </>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className={cn(
                  "w-full text-left p-3 rounded-lg border-2 transition-all",
                  "hover:border-primary hover:bg-accent",
                  currentGoalId === selectedGoalId && !currentMilestoneId ? "border-primary bg-accent" : "border-border"
                )}
              >
                <div className="flex items-start gap-2">
              ...
              {filteredMilestones.length > 0 && (
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-background px-2 text-muted-foreground">Or select a milestone</span>
                  </div>
                </div>
              )}
                  ...
                  {filteredMilestones.map((milestone) => (
                    <button
                      ...
                      className={cn(
                        "w-full text-left p-3 rounded-lg border-2 transition-all",
                        "hover:border-primary hover:bg-accent",
                        currentMilestoneId === milestone.id ? "border-primary bg-accent" : "border-border"
                      )}
                    >
                      <div className="flex items-start gap-2">
