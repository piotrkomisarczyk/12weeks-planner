...
export function useWeekPlan(planId: string, weekNumber: number): UseWeekPlanReturn {
  ...
  const addWeeklyGoal = useCallback(
    async (title: string, longTermGoalId?: string, milestoneId?: string) => {
  ...
  const updateWeeklyGoal = useCallback(
    async (id: string, updates: Partial<UpdateWeeklyGoalCommand>) => {
      // Optimistic update
   ...
   */
  const reorderWeeklyGoals = useCallback(
    async (newOrder: WeeklyGoalViewModel[]) => {
      const previousData = { ...data };
   ...
   */
  const moveWeeklyGoalUp = useCallback(
    async (id: string) => {
   ...
   */
  const moveWeeklyGoalDown = useCallback(
    async (id: string) => {
      const currentIndex = data.weeklyGoals.findIndex((g) => g.id === id);
      ...
      if (weeklyGoalId) {
        setData((prev) => ({
          ...
          weeklyGoals: prev.weeklyGoals.map((g) =>
            g.id === weeklyGoalId ? { ...g, tasks: [...g.tasks, optimisticTask] } : g
          ),
        }));
      } else {
        setData((prev) => ({
      ...
      try {
        ...
        if (weeklyGoalId) {
          setData((prev) => ({
            ...prev,
            weeklyGoals: prev.weeklyGoals.map((g) =>
              g.id === weeklyGoalId ? { ...g, tasks: g.tasks.map((t) => (t.id === tempId ? newTask : t)) } : g
            ),
          }));
        } else {
          setData((prev) => ({
        ...
        if (weeklyGoalId) {
          setData((prev) => ({
            ...prev,
            weeklyGoals: prev.weeklyGoals.map((g) =>
              g.id === weeklyGoalId ? { ...g, tasks: g.tasks.filter((t) => t.id !== tempId) } : g
            ),
          }));
        } else {
          setData((prev) => ({
  ...
  const moveTask = useCallback(
    async (taskId: string, sourceContainerId: string | null, destContainerId: string | null, newIndex: number) => {
      const previousData = { ...data };
      ...
      if (sourceContainerId) {
        newWeeklyGoals = newWeeklyGoals.map((g) =>
          g.id === sourceContainerId ? { ...g, tasks: g.tasks.filter((t) => t.id !== taskId) } : g
        );
      } else {
