...
export function useWeeklyReview({
  ...
  const updateReflection = useCallback((
    ...
    const getTimerRef = () => {
      switch (field) {
        case 'what_worked': return whatWorkedTimerRef;
        case 'what_did_not_work': return whatDidNotWorkTimerRef;
        case 'what_to_improve': return whatToImproveTimerRef;
      }
    };
   ...
   */
  const updateGoalProgress = useCallback(async (goalId: string, newProgress: number) => {
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        goals: prev.goals.map(g =>
          g.id === goalId ? { ...g, isUpdating: false } : g
        )
      }));
